...
from ...models.query import QueryResponse, UserQuery
...
def get_query_processor(
    ...
    llm_client = OllamaClient(base_url=settings.ollama_base_url, model=settings.ollama_model)
...
async def submit_query(
    ...
    try:
        ...
        response_text, search_results, processing_time_ms = await query_processor.process_query(
            query_text=request.query_text, session=session
        )
...
async def submit_query_stream(
    ...
    async def generate_stream() -> AsyncGenerator[str, None]:
        try:
            ...
            if (
                query_processor.use_hybrid_search
                and query_processor.vectordb_client.enable_hybrid_search
            ):
                ...
                search_results = query_processor.vectordb_client.search_similar(
                    query=request.query_text, n_results=query_processor.num_results
                )
            ...
            if session and len(session.messages) > 0:
                ...
                prompt = build_rag_prompt(query=request.query_text, search_results=search_results)
            ...
            logger.info(f"Streaming query completed. Processing time: {processing_time_ms:.2f}ms")
