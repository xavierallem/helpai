...
class LegalDocument(BaseModel):
    ...
    processing_status: ProcessingStatus = Field(
        default=ProcessingStatus.PENDING, description="Current processing status of the document"
    )
    uploaded_at: datetime = Field(
        default_factory=datetime.utcnow, description="Timestamp when document was uploaded"
    )
    processed_at: datetime | None = Field(
        default=None, description="Timestamp when document processing completed"
    )
    error_message: str | None = Field(
        default=None, description="Error message if processing failed"
    )
    total_chunks: int = Field(
        default=0, description="Total number of chunks created from this document"
    )
